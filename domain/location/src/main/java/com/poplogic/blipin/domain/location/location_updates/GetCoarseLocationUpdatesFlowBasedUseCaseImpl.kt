...
class GetCoarseLocationUpdatesFlowBasedUseCaseImpl(
    ...
    override fun invoke(param: Unit): Flow<Location> =
        callbackFlow {
            val canLocationServiceBeenUsedUseCaseResult =
                canLocationServiceBeenUsedUseCase.invoke(Unit)
            when (canLocationServiceBeenUsedUseCaseResult.getOrNull()) {
                null -> {
                    close(IllegalStateException("Unknown error"))
                    return@callbackFlow
                }

                else -> {
                    if (!canLocationServiceBeenUsedUseCaseResult.getOrNull()!!.canUseCoarseLocation) {
